...
class EvaluateAbilityToIdentifyTopTestSamples:
    def __init__(self, percentage_of_top_samples, y_train_with_predicted_test,
                 all_data):
        self.y_pred_all = y_train_with_predicted_test
        self.y_true_all_all = all_data["y_true_all"]
        self.y_true_all = all_data["y_true"]
    ...
    def find_sum_of_estimates_of_top_x_tests(self, y_pred_all):
        # a list of sample IDs in the descending order of activity values
        orders_overall = np.argsort(-y_pred_all)
        ...
        for test_id in top_x_tests:
            sum_y_true_of_identified_test.append(self.y_true_all_all[test_id])
    ...
    def find_top_test_ids(self, y_pred_all, Y_sign_and_abs_predictions=None):
        ...
        top_trains_and_tests = overall_orders[0:int(len(self.test_ids))]
        ...
        try:
            ...
            tests_better_than_top_train = list(overall_orders)
    ...
    def estimate_y_from_final_ranking_and_absolute_Y(self, top_tests, tests_better_than_top_train, top_train_id,
        ...
        for test_id in set(top_tests + tests_better_than_top_train):
            ...
            Y_ave = y_test_estimate - self.y_true_all_all[top_train_id]
            final_estimate_of_y_and_delta_y[test_id] = [Y_rough, Y_ave, self.y_true_all_all[test_id], y_test_estimate]
    ...
    def run_evaluation(self, Y_sign_and_abs_predictions=None):
        ...
        if len(top_tests_true) > 0:
            ...
            self.x = int(0.2*len(self.all_data["test_ids"])) if len(self.test_ids) >= 5 else 1
            sum_y_true_of_pred_top_test = self.find_sum_of_estimates_of_top_x_tests(self.y_pred_all)
            sum_y_true_of_true_top_test = self.find_sum_of_estimates_of_top_x_tests(self.y_true_all)

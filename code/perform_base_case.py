...
def generate_meta_data(all_data):
    ...
    """
    # regression on FP
    _, y_SA = regression(RandomForestRegressor(random_state=1, n_jobs=-1), all_data['train_set'], all_data['test_set'])

    # regression on pairs of FP for C2 and C3 type test pairs
    _, Y_pa_c2 = regression(RandomForestRegressor(random_state=1, n_jobs=-1), all_data['train_pairs'], all_data['c2_test_pairs'])

    _, Y_pa_c3 = regression(RandomForestRegressor(random_state=1, n_jobs=-1), all_data['train_pairs'], all_data['c3_test_pairs'])

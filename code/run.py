...
if __name__ == '__main__':
    ...
    try:
        existing_results = np.load("extrapolation_kfold_cv_all_data_rf_rerun2.npy")
    ...
    try:
        _ = np.load("extrapolation_temporary_dataset_count_rf_rerun2.npy")
    except:
        np.save("extrapolation_temporary_dataset_count_rf_rerun2.npy", [0])
    ...
    for file in range(len(chembl_info)):
        ...
        with open('dataset_running_order_rf_rerun2.txt', 'a') as f:
            f.write(filename + "\n")
        ...
        np.save("extrapolation_kfold_cv_all_data_rf_rerun2.npy", np.array(all_metrics))

...
def performance_pairwise_approach(all_data, percentage_of_top_samples, batch_size=400000):
...
def run_model(data, current_filename, percentage_of_top_samples):
    try:
        existing_iterations = np.load(os.getcwd() + "/extrapolation_xgb_chembl/"+ "extrapolation_xgb_chembl_kfold_cv_all_data_temporary_"+str(current_filename)+".npy")
    ...
    for outer_fold, datum in data.items():
        ...
        np.save(os.getcwd() + "/extrapolation_xgb_chembl/"+ "extrapolation_xgb_chembl_kfold_cv_all_data_temporary_"+str(current_filename)+".npy", np.array(metrics))
